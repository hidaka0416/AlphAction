...
def main():
    ...
    try:
        # hidaka edit
        df = pd.DataFrame()
        #img_dir_path = args.output_path + '_jpgs/'
        ...
        for i in tqdm(count(), desc="Tracker Progress", unit=" frame"):
            with torch.no_grad():
                ...
                #img_path = img_dir_path + 'img_{:0=6}.jpg'.format(i)
                #img.save( img_path )
                if boxes is None:
                    df = df.append( pd.DataFrame( { 'frame_no' : i,
                                                    #'img_path' : img_path,
                                                    'x1' : None,
                                                    'y1' : None,
                                                    'x2' : None,
                    ...
                    #print( '\nboxes :{}, type :{}'.format( boxes.tolist(), type( boxes.tolist() ) ) )
                    boxes = boxes.tolist()
                    person_ids = ids.tolist()
                    for box_num in range( len( boxes ) ):
                        bbox = boxes[box_num]
                        person_id = person_ids[box_num] 
                        df = df.append( pd.DataFrame( { 'frame_no' : i,
                                                        #'img_path' : img_path,
                                                        'x1' : bbox[0],
                                                        'y1' : bbox[1],
        ...
        print( df )
        df.to_csv( args.output_path + '_tracker.csv' )
        # 
